...
quantization_config = BitsAndBytesConfig(
    load_in_8bit=True, llm_int8_skip_modules=["lm_head"], llm_int8_threshold=3.0
)
...
def create_or_load_tokenizer(checkpointed_path=None):
    if checkpointed_path:
        ...
        tokenizer = AutoTokenizer.from_pretrained(
            config["MODEL_NAME"], cache_dir=f"{DATA_PATH}/datasets"
        )
...
def fetch_optimizer(model):
    ...
    head_layers = set(
        [
            ...
            "base_model.model.lm_head.0.weight",
        ]
    )
...
def create_or_load_model(
    ...
    quantized=config["PEFT"],
    frozen=False,
    cast_layer_norm_to_fp32=False,
    cast_output_to_fp32=False,
):
...
def inference(model, tokenizer, device, quantized=config["PEFT"]):
    ...
    generated_text = (
        tokenizer.decode(output_sequence[0], skip_special_tokens=True)
        .replace("\n", "")
        .replace("\t", " ")
    )
...
def validate(model, device, valid_dataloader):
    ...
    for index, batch in tqdm(enumerate(valid_dataloader, 1)):
        ...
        logging.info(
            f"Validation: Batch {index}/{len(valid_dataloader)}, Loss: {avg_eval_loss:.4f}"
        )
    ...
    for epoch in tqdm(range(start_epoch, config["NUM_EPOCHS"] + 1)):
        ...
        for index, batch in tqdm(
            ...
            if index % config["CHECKPOINTING_INTERVAL"] == 0:
                logging.info(
                    f"Checkpointing model at epoch={epoch} and batch={index}\n"
                )
            ...
            if index % config["VALIDATION_INTERVAL"] == 0:
                ...
                logging.info(
                    f"Batch {index}/{len(train_dataloader)}, Validation Loss: {avg_eval_loss:.4f}, Perplexity: {perplexity:.2f}"
                )
            ...
            logging.info(
                f"Batch {index}/{len(train_dataloader)}, Loss: {avg_loss:.4f}, Learning Rate: {learning_rate}"
            )
        ...
        if epochs_since_min_loss >= patience:
            logger.info(
                "Early stopping triggered. No improvement in training loss for {} epochs.".format(
                    patience
                )
            )
            break
...
if __name__ == "__main__":
    ...
    if config["PEFT"]:
        model, device = create_or_load_model(
            checkpointed_path=checkpointed_path,
            frozen=True,
        ...
        print_trainable_parameters(model)
        logger.info(
            f"Memory Memory Footprint: {model.get_memory_footprint() / 1e6:,} MB"
        )
        logger.info(f"Model is on device: {model.device}")
    else:
