...
def store_god_configurations_if_not_exists(db_path, god_tag):
    ...
    flat_configs = extract_god_configs_to_flat_dict()
...
def store_checkpoint(db_path, epoch, index, run_name, path):
    ...
    cursor.execute(
        'INSERT INTO CheckpointPaths (TIMESTAMP, EPOCH, INDEX, RUN_NAME, PATH) VALUES (?, ?, ?, ?, ?)',
        (timestamp, epoch, index, run_name, path)
    )
    conn.commit()
